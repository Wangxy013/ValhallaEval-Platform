...
- **三阶段模型配置**：任务创建时可分别指定推理模型、校验模型和评估模型，任务详情页同步展示阶段模型
- **中文校验裁判**：校验 prompt 现强制结合校验点名称、校验标准和模型输出，并要求裁判以中文 JSON 返回结果和理由
- **API Key 脱敏展示**：模型列表、详情、创建/更新响应中的密钥均改为脱敏值，编辑时默认不回填明文
- **DB 迁移**：`tasks` 表新增 `validation_model_id`、`assessment_model_id` 两个阶段模型字段

### 修复

- 模型编辑时若 API Key 留空，不再误覆盖已保存密钥
- 任务结果页的校验说明优先读取结构化中文说明，不再直接暴露原始裁判输出
