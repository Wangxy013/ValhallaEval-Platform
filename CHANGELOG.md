...
所有重要变更都会记录在本文件中。格式基于 [Keep a Changelog](https://keepachangelog.com/zh-CN/1.0.0/)，版本遵循 [Semantic Versioning](https://semver.org/lang/zh-CN/)。
...
- **基准 Prompt 选择**：Prompt 对比模式下，可在创建任务时指定一个 Prompt 作为对比基准，评测结果围绕该基准展开
- **基准标注**：总览页矩阵、明细页分组展示中，基准版本均显示金色「基准」徽章，差异分析标注改善/退步以基准为参照
- **基准排序**：明细页分组展开时，基准版本的运行结果始终排在第一位
- **DB 迁移**：`task_prompts` 表新增 `is_baseline BOOLEAN` 列（默认 FALSE）
...
- **测试数据选择**：`execute_task` 改为使用任务创建时快照的 `task_test_items`，不再忽略用户勾选
